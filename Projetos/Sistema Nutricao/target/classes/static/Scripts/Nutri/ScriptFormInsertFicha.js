document.addEventListener("DOMContentLoaded", function () {
  ...
  window.adicionarIngrediente = function () {
    ...
    row.innerHTML = `
      ...
      2
            ...
            <td class="px-4 py-2 border border-black text-center">
              <button type="button" onclick="removerIngrediente('ingrediente_${idx}', '${ingredienteId}')"
  ...
  function initCalculoFC() {
    ...
    function atualizarFC() {
      ...
      fcValueInput.value = fc.toFixed(2);
    }
  ...
  function initAguaCheckbox() {
    ...
    if (checkbox && aguaInputs) {
      checkbox.addEventListener("change", function () {
        ...
        // Se o checkbox não estiver marcado, limpa os campos de água
        if (!this.checked) {
          if (qntdAguaInput) qntdAguaInput.value = "";
          if (porcentAguaInput) porcentAguaInput.value = "";
        }
      });
    }
  }
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      if (document.querySelectorAll(".ingrediente-row").length === 0) {
        e.preventDefault();
        alert(
          "Adicione pelo menos um ingrediente antes de enviar o formulário!"
        );
        return;
      }
